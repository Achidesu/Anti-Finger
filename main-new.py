...
class AntiTriggerFingersApp(ctk.CTk):
    def __init__(self):
        ...
        self.time_max = 5
        self.time_current = 0
        # start from 6 and then step down once to 5 (6 -> 5)
        self.hand_posit = 6
    ...
    def timer_reset(self):
        self.time_current = self.time_max
        self.time_remaining = self.time_max
        # start the hand picture at 6 (then will step to 5)
        self.hand_posit = 6
        self.last_update_time = time.time()
        self.update_timer()
        self.reset_pic()
    ...
    def reset_pic(self): 
        # Reset to pose_1 image index 6 by default
        robot_hand_image_pil = Image.open(f"pictures/pose_1/6.jpg")
    ...
    def start_pose_countdown(self, count=3):
        def countdown_thread():
            ...
            if self.running:
                # Countdown finished -> setup main loop
                self.timer_canvas.delete("progress")
