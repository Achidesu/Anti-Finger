    ...
    def on_key_press(self, event=None):
        ...
        if self.key == "1":
            if self.current_pose == 1:
                ...
                self.timer_canvas.delete(self.timer_text)
                self.timer_text = self.timer_canvas.create_text(100, 100, text=f"{self.time_current}",
                                                                font=self.font_timer, fill=self.black_fg)
                print(f"[Debug] from 1 : {self.time_current}")
                sleep(1)
