...
class AntiTriggerFingersApp:
    def __init__(self, master):
        self.master = master
        master.title("AI-Powered Anti-trigger Fingers")
        master.geometry("1280x800+0+0")
        master.resizable(False, False) # Prevent resizing for fixed layout
        ...
        # --- Top Bar (Header) ---
        self.top_bar_frame = ctk.CTkFrame(root, fg_color=self.purple_bg, height=150)
        ...
        # --- Main Content Area ---
        self.main_content_frame = ctk.CTkFrame(root, fg_color=self.light_gray_bg_program)
    ...
    def reset_action(self):
        ...
        # Add functionality for the reset button here

    def change_pic(self):
        small_hand_image_pil = Image.open(run_name)  # Assuming 'small_hand.png'
        small_hand_image_pil = small_hand_image_pil.resize((200, 200), Image.LANCZOS)  # Adjust size
        self.small_hand_photo = ImageTk.PhotoImage(small_hand_image_pil)
        self.small_hand_label = ctk.CTkLabel(self.main_content_frame, image=self.small_hand_photo, text="")
        self.small_hand_label.grid(row=1, column=2, padx=20, pady=(0, 20), sticky="n")
    ...
    def on_key_press_1(event):
        for i in range(5):
            run_name = f"pictures/pose_1/{i}.jpg"
            print(run_name)
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    root.bind("<Key-1>",AntiTriggerFingersApp.on_key_press_1)
    app = AntiTriggerFingersApp(root)
    root.mainloop()